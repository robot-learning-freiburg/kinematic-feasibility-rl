...
tf::Transform BaseWorld::get_base_transform_world() {
    if (name_ != "sim") {
...
void SimWorld::set_model_state(std::string model_name,
...
void GazeboWorld::set_model_state(std::string model_name,
                                  tf::Transform world_transform,
                                  RoboConf robo_config,
                                  ros::Publisher &cmd_base_vel_pub) {
...
bool RealWorld::is_within_world(tf::Transform base_transform) {
...
void RealWorld::set_model_state(std::string model_name,
                                tf::Transform world_transform,
                                RoboConf robo_config,
                                ros::Publisher &cmd_base_vel_pub) {
    if (model_name != "pr2") {
    ...
    tf::Transform currentBaseTransform = get_base_transform_world();
    tf::Vector3 goal_vec = world_transform.getOrigin() - currentBaseTransform.getOrigin();

    while (goal_vec.length() > 0.025) {
        // location
        tf::Vector3 scaled_vel = utils::norm_scale_vel(goal_vec, 0.0, robo_config.base_vel_rng);
        ...
        double base_rotation = utils::clamp_double(angle_diff, -robo_config.base_rot_rng, robo_config.base_rot_rng);
        ...
        // update
        currentBaseTransform = get_base_transform_world();

...
GripperPlan LinearPlanner::get_next_step(
        const GripperPlan &prevPlan,
        const double &dt
    ){
    GripperPlan plan;
    double min_vel = min_planner_velocity_ * dt;
    double max_vel = max_planner_velocity_ * dt;
    ...
    if (use_base_goal_){
        planned_base_vel = get_vel(prevPlan.nextBaseTransform, baseGoal_, min_vel, max_vel);
    } else {
    ...
    tf::Quaternion planned_gripper_q = get_rot(initialGripperTransform_,
                                               plan.nextGripperTransform,
                                               gripperGoal_,
                                               initial_dist_to_gripper_goal_);
    ...
    // tf::Quaternion next_base_q = get_rot(prevPlan.nextBaseTransform, prevPlan.nextBaseTransform, ggoal_floor, (ggoal_floor.getOrigin() - initialBaseTransform_.getOrigin()).length());
    // plan.nextBaseTransform.setRotation(next_base_q);
    return plan;
}

GripperPlan LinearPlanner::get_next_velocities(
        double time,
        double dt,
...
//    utils::print_t(prevPlan_.nextBaseTransform, "prevPlan_.nextBaseTransform");
    ...
    GripperPlan nextPlan = get_next_step(prevPlan_, dt);
    if (update_prev_plan){
        prevPrevPlan_ = prevPlan_;
        prevPlan_ = nextPlan;
    }
...
GripperPlan LinearPlanner::get_velocities_from_prevPrev(
        double time,
        double dt,
    ...
    return get_next_step(prevPrevPlan_, dt);
}
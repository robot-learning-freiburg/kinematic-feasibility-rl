...
from modulation.utils import calc_dist_to_sol
...
def evaluate_policy(model, env, n_eval_episodes=10, deterministic=True,
    ...
    episode_rewards, episode_returns, episode_lengths, episode_actions, kin_fails, pathPoints, dist_gripper_sols_success, dist_gripper_sols_fail, dist_gripper_sols_max, collisions, final_dist_to_goal = [], [], [], [], [], [], [], [], [], [], []
    ...
    with torch.no_grad():
        for i in range(n_eval_episodes):
            ...
            episode_kin_fails, info, episode_action = [], [{}], []
            while not done:
                action, state = model.predict(obs, state=state, deterministic=deterministic)
                base_actions = env.env_method('_convert_policy_to_env_actions', action[0])[0]
                episode_action += [base_actions]

                if debug:
                    assert not np.isnan(episode_action[-1]).sum(), f"Nan found in actions: {episode_action[-1]}"
    ...
    actions = np.concatenate(episode_actions)

    if dist_gripper_sols_success:
        log_dict[f"{name_prefix}/dist2gripperSol_success"] = np.mean(dist_gripper_sols_success)
    if dist_gripper_sols_fail:
        log_dict[f"{name_prefix}/dist2gripperSol_fail"] = np.mean(dist_gripper_sols_fail)
    log_dict[f"{name_prefix}/dist2gripperSol_below0.1_plusReached"] = ((np.array(final_dist_to_goal) <= 0.1) * (np.array(dist_gripper_sols_max) < 0.1)).mean()
    log_dict[f"{name_prefix}/dist2gripperSol_below0.05_plusReached"] = ((np.array(final_dist_to_goal) <= 0.05) * (np.array(dist_gripper_sols_max) < 0.05)).mean()

    # log_dict[f"{name_prefix}/collisions"] = np.mean(collisions)
    # log_dict[f"{name_prefix}/collisions_zero"] = (np.array(collisions) == 0).sum() / n_eval_episodes
    # log_dict[f'{name_prefix}/collisions_hist'] = wandb.Histogram(np_histogram=np.histogram(collisions, bins=64, density=True))

    fig_pathPoints = plot_pathPoints(pathPoints[:plot_n_path_points] if plot_n_path_points else pathPoints)
    log_dict[f"{name_prefix}/pathPoints"] = wandb.Image(fig_pathPoints)

    fig_relPose2d, fig_relPose3d = plot_relative_pose_map(pathPoints)
    log_dict[f"{name_prefix}/relPose2D"] = wandb.Image(fig_relPose2d)
    log_dict[f"{name_prefix}/relPose3D"] = wandb.Image(fig_relPose3d)

    f_zfailure_hist = plot_zfailure_hist(pathPoints)
    log_dict[f"{name_prefix}/zfailure_hist"] = wandb.Image(f_zfailure_hist)

    fails_per_episode = np.array(kin_fails)
    final_dist_to_goal = np.array(final_dist_to_goal)
    log_dict[f'{name_prefix}/kin_fails_hist'] = wandb.Histogram(np_histogram=np.histogram(fails_per_episode, bins=64, density=True, range=(0, ik_fail_thresh_eval)))
    for a, name in enumerate(env.get_attr('action_names')[0]):
        log_dict[f'actions_{name_prefix}/{name}'] = wandb.Histogram(np_histogram=np.histogram(actions[:, a], bins=64, density=True))
...
class IKSlackScheduleCallback(EventCallback):
    ...
    def _on_step(self) -> bool:
        if self.training_env.get_attr('_env_name')[0] == 'hsr':
            ...
            self.training_env.env_method('set_ik_slack',
                                         lin_sched(progress_remaining, self._start_dist_slack, self._end_dist_slack),
